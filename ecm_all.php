...
<?php
 ...
 $month=$end; include ("time.inc"); $month_end=$month;
 $cn=1;
 ...
 $query = 'SELECT SUM(value) FROM data2 WHERE prm=1 AND channel=50 AND type=4 AND date>='.$date1;
 //echo $query;
 if ($e6 = mysql_query ($query,$i)) if ($ui6 = mysql_fetch_row ($e6))
 if ($ui6[0]) $eksum_nach=$ui6[0];
 $eksum_nach=3141245.89;

 $query = 'SELECT SUM(value) FROM data2 WHERE prm=1 AND channel=50 AND type=4';
 //echo $query;
 if ($e6 = mysql_query ($query,$i)) if ($ui6 = mysql_fetch_row ($e6))
...
// echo $data3; 
 ...
 print '<table border="0" cellpadding="0" cellspacing="0" style="padding-left:5px" width="800px">
	...
	<tr><td>Директор ООО "ЮАИЗ-ПФИ"</td><td></td><td>Директор ООО "ТТ"</td></tr>
	...
	<tr><td style="height:60px"></td></tr>
	<tr><td align="left"><font class="head4"> 1. Отчётный период: с "01" '.$month_begin.' '.$_GET["styear"].' г. по "01" '.$month_end.' '.$today["year"].' г.</font></td></tr>
	...
	<tr class="GridItemStyle"><td>5.</td><td>Сточные воды</td><td >'.$ek4.'</td><td >'.(number_format($ek4*$tarif_vodootv,2,'.','')).'</td><td >'.$tarif_vodootv.' руб.м3</td></tr>
...
if (!$_GET["print"]) 
    {
     ...
     print '</td>
	    <td><font class="head3">Согласовано со стороны ООО "ТТ"</font></td><td><font class="head3">'.$visa_date2.'</font></td><td>';
