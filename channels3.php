 ...
 while ($ui = mysql_fetch_row ($e))
	{
	 ...
	 while ($ui2 = mysql_fetch_row ($e2))
		{
	 	 $device=$ui2[1];
		 $query = 'SELECT * FROM channels WHERE opr>0 AND (prm=14 AND pipe=21) AND device='.$device;
		 $cn=0;
		 ...
		 while ($uo = mysql_fetch_row ($e3))
			{
			 ...
			 if ($cn%2) print '<tr class="alter-row"><td class="left"><a href="index.php?sel=report" title="'.$uo[1].'">'.$ui2[20].' | '.$uo[1].' ('.$uo[0].')</a></td>';
		         else print '<tr class="row"><td class="left"><a href="index.php?sel=report" title="'.$uo[1].'">'.$ui2[20].' | '.$uo[1].' ('.$uo[0].')</a></td>';

			 $query = 'SELECT * FROM prdata WHERE type=0 AND device='.$device.' AND channel='.$uo[0].' ORDER BY date DESC LIMIT 2';
			 if ($e4 = mysql_query ($query,$i))
			 if ($uo2 = mysql_fetch_row ($e4))
				{
				 $pvalue1=$uo2[5]; $ptime1=$time=$uo2[4];
				}

			 $query = 'SELECT COUNT(id) FROM hours WHERE type=9 AND device='.$device.' AND channel='.$uo[0];
			 if ($e4 = mysql_query ($query,$i))
			 if ($uo2 = mysql_fetch_row ($e4))
				{
				 $count=$uo2[0];
			 ...
			 if ($count>2)
			    {
	                     ...
	                     print '<td class="right">'.number_format($pvalue1,4).'('.$count.')</td>';
	                    }
			 else
			    {
