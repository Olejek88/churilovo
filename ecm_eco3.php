	...
	while ($ui = mysql_fetch_array ($e, MYSQL_ASSOC))
		{
		 ...
		 //for ($tn=1; $tn<=$qnt; $tn++) if ($ui["date"]==$date[$tn]) $x=$tn;
		 for ($tn=1; $tn<=$qnt2; $tn++)
    	 	    if ($ui["channel"]==$vens[$tn]) $data_ven[$tn][$x]+=$ui["value"];
    		 for ($tn=1; $tn<=$qnt2; $tn++)
    		     if ($ui["channel"]==$gors[$tn]) $data_gor[$tn][$x]+=$ui["value"];
			 ...
			 for ($tn=1; $tn<=$qnt; $tn++)
			    {
			     ...
			     $h[$tn]=0;
			     if ($w[$tn]==0) $w[$tn]+=$elecs[$ccn]*$data_ven[$ccn][$tn];
			     if ($g[$tn]==0) $g[$tn]+=$gass[$ccn]*$data_gor[$ccn][$tn];
	    		     //if ($tn==1) echo '['.$w.'] '.$elec.' '.$data_elec[$ccn][$tn].' '.$data_ven[$ccn][$tn].' '.$sum_ven[$ccn][$tn].'<br>';
			    }
			 $ccn++; $cn++;
			}
		 for ($tn=1; $tn<=$qnt; $tn++)
		 ...
		 while ($ui3 = mysql_fetch_row ($e3))
			{
			 ...
			 if (!$_GET["hide"])
				...
				else print '<td class="simple" align="center"><input style="background-color:pink" id="'.$ccn.'-'.$tn.'-2-0" name="'.$ccn.'-'.$tn.'-2-0" class="simple2" value="'.number_format($data_gor[$ccn][$tn],3).'" align="center" style="background:#dedede"></td>';
			     for ($tn=1; $tn<=$qnt; $tn++)
				...
				else print '<td class="simple" align="center"><input style="background-color:pink" id="'.$ccn.'-'.$tn.'-2-1" name="'.$ccn.'-'.$tn.'-2-1" class="simple2" value="'.number_format($data_ven[$ccn][$tn],3).'" align="center" style="background:#dedede"></td>';
  ...
  while ($ui = mysql_fetch_array ($e, MYSQL_ASSOC))
	{
	 ...
	 //echo $date[$tn].'<br>';
 	 if ($ui["prm"]==41 && $ui["source"]==2) $data1[$x]=$ui["value"];
 	 if ($ui["prm"]==41 && $ui["source"]==0) $data2[$x]=$ui["value"];
	 if ($ui["prm"]==12 && $ui["source"]==0) $data3[$x]+=$ui["value"];
