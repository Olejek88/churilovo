...
$query = 'SELECT * FROM data WHERE type='.$type.' AND channel='.$_GET["chan"].' ORDER BY date DESC LIMIT 200';
...
while ($uy = mysql_fetch_row ($a))
 ...
 for ($i2=0; $i2<$max; $i2++) 
	{ 	 
	 ...
	 if ($_GET["type"]==2) $dat[$i2]=$dats[$max-$i2][5].$dats[$max-$i2][6].'-'.$dats[$max-$i2][8].$dats[$max-$i2][9]; 
	 $data0[$i2]=$datas0[$max-$i2];
	}

// Create the graph. These two calls are always required
if ($_GET["x"]!='') $graph = new Graph($_GET["x"],$_GET["y"],"auto");
else $graph = new Graph(1250,200,"auto");	
...
$graph->xaxis->SetTextTickInterval(20,0);
