...
$query = 'SELECT * FROM data WHERE type=1 AND value<1.1 AND channel='.$gor.' ORDER BY date DESC LIMIT '.$max;
//echo $query;
...
$query = 'SELECT * FROM data WHERE type=1 AND value<1.1 AND channel='.$ven.' ORDER BY date DESC LIMIT '.$max;
//echo $query; 
$rr=0;
if ($u = mysql_query ($query,$i)) 
while ($uo = mysql_fetch_row ($u))
