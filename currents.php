...
<td>Узел учета</td><td>Параметр</td><td>Время чтения</td><td align="center">Текущее</td><td align="center">Суточное</td><td align="center">Месяц</td><td align="center">Ед.Из.</td><td style="width: 10px;">&nbsp;</td></tr>
<?php
	 ...
	 while ($ui)
		{
		 ...
		 $query = 'SELECT * FROM prdata WHERE device='.$uy[1].' AND type<9 AND channel='.$ui[0].' ORDER BY date DESC';
		 $r = mysql_query ($query,$i);
		 if ($r) $uy2 = mysql_fetch_row ($r);
		 while ($uy2)
			{
			 if ($uy2[3]==0 && $current=='-') { $current=$uy2[5]; $time=$uy2[4]; }
			 if ($uy2[3]==2 && $day=='-') { $day=$uy2[5]; $time2=$uy2[2]; }
			 else { $hour2=$uy2[5]; if ($hour2!=0) $del=($hour-$hour2)*100/$hour2; else $del=0; }
			 if ($uy2[3]==4 && $month=='-') { $month=$uy2[5]; $time3=$uy2[2]; }
			 $uy2 = mysql_fetch_row ($r);
			}
		 print '
		 <tr style="color: Black; background-color: White; font-family: Verdana; font-size: xx-small;">
		 <td><a target="_top" href="index.php?sel=device&id='.$uy[1].'">'.$place.' / '.$uy[20].'</a></td>
		 <td><a target="_top" href="index.php?sel=channel&type=9&id='.$ui[0].'">'.$ui[1].'</a></td>
		 <td>'.$time.'</td>';
		 if ($current!='-') print '<td align="center">'.number_format($current,3).'</td>';
		 else print '<td align="center"></td>';
		 if ($day!='-') print '<td align="center">'.number_format($day,3).' ['.$time2.']</td>';
		 else print '<td align="center"></td>';
		 if ($month!='-') print '<td align="center">'.number_format($month,3).' ['.$time3.']</td>';
		 else print '<td align="center"></td>';
